...
[A_hat,time_value_bsd] = batch_bcd(P, Q, A_init, K_bcd, R, Y, omega_t, omega_l, lambdastar, lambda1, mu_soft_bcd, R*Z);
[AA_hat, time_value_bsca] = bsca_missing_data(P, Q, A_init, K_bsca, R, Y, omega_t, omega_l, lambdastar, lambda1, mu_soft_bsca, R*Z);

[A_hat8,time_value_bsd8] = batch_bcd(P, Q, A_init, K_bcd, R, Y, omega_t1, omega_l1, lambdastar, lambda1, mu_soft_bcd, R*Z);
[AA_hat8, time_value_bsca8] = bsca_missing_data(P, Q, A_init, K_bsca, R, Y, omega_t1, omega_l1, lambdastar, lambda1, mu_soft_bsca, R*Z);
...
A8=abs(A);
figure(1);
hold on;set(gca,'Position',[.13 .17 .80 .74]); 

subplot(2,1,1)
for i =1:10
plot(A(:,i),'ob-');hold on
plot(A_hat(:,i),'Xr-');hold on
plot(AA_hat(:,i),'+k-');hold on
end
xlabel('time index'); 
ylabel('Anomaly amplitude');
box on;
legend('true anomalies','BCD estimated','BSCA estimated')
title(' with \pi = 0.8')

subplot(2,1,2)

for j =1:10
plot(A8(:,j),'ob-');hold on
plot(A_hat8(:,j),'Xr-');hold on
plot(AA_hat8(:,j),'+k-');hold on
end
xlabel('time index'); 
ylabel('Anomaly amplitude');
box on;
...
function [A_hat1,timeValue] = batch_bcd(P, Q, A, K, R, Y, omega_t, omega_l, lambdastar, lambda1, mu_soft, Z)
...
function [A_hat2, timeValue] = bsca_missing_data(P, Q, A, K, R, Y, omega_t, omega_l, lambdastar, lambda1, mu_soft, Z)
